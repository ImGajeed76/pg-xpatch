...
--     inside batch_mutex to prevent straggler escape between unlock and increment
--   - Fixed: reversed counter reset order (completed before next_task) to close
--     the window where a straggler could see a reset next_task with stale completed
--   - Added diagnostic tracking (task_completed[], entry_count) for debugging
